...
static void apply_to_print_region_config(PrintRegionConfig &out, const DynamicPrintConfig &in, FeatureFilamentOverrideMask &feature_overrides)
{
    ...
    // Orca: surface_wall_override_filament is NOT in FeatureFilamentOverrideMask — it is
    // additive (per-volume/layer-range extruder sets the BASE filament; the override still
    // applies on top of it for the outermost N loops). Step 3 only auto-applies base_extruder
    // to tracked features, so the override is naturally left alone.
