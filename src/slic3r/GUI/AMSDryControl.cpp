        ...
        for (auto filament_it = filaments.begin(); filament_it != filaments.end(); ++filament_it) {
            ...
            for (auto printer_str : printer_strs->values) {
                if (printer_names.find(printer_str) != printer_names.end()) {
                    ...
                    // BBS calls `filaments.get_preset_alias(*filament_it, true)`
                    // here — Orca's PresetCollection has no such method, so
                    // we fall back to an empty alias. The surrounding
                    // `!filament_alias.empty()` gate short-circuits the
                    // preset-info lookup, which we don't have data for either.
...
DevFilaSystem* AMSDryCtrWin::get_fila_system() const
{
    DevFilaSystem* fila_system = m_fila_system;
    if (!fila_system) {
...
void AMSDryCtrWin::update(DevFilaSystem* fila_system, MachineObject* obj)
{
    if (!fila_system || !obj) {
        return;
    }
