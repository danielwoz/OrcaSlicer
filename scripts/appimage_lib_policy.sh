...
appimage_is_host_library() {
    ...
    case "$lib_name" in
        ...
        libssl.so*|libcrypto.so*|libcurl.so*)
            # OpenSSL 3 + libcurl: required by the bundled open-bamboo-networking
            # plugin (resources/plugins/), which links the system copies. These are
            # baseline on every modern Linux (Ubuntu 22.04+/Fedora), and the plugin
            # is optional, so resolving them from the host is acceptable.
            return 0
            ;;
        *)
            return 1
